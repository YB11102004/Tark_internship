                ...
                if (d.equals("exit")) {
                    ...
                    if (selectedTrain.areCitiesAvailable(src, dest)) {
                        ...
                        if (availableSeats != null) {
                            ...
                            System.out.print(tktnum + " " + Fare);
                            if(confirmedSeatsCount!=0){
                                System.out.print(" CNF/" + confirmedSeatsCount);
                            }
                            if(waitingListCount!=0){
                                System.out.print(" WL/" + waitingListCount);
                            }
                            System.out.println();
...
class Train {
    ...
    List<RouteWaitingList> routeWaitingLists = new ArrayList<>();
    public List<String> checkSeatAvailability(String seatClass, LocalDate travelDate, String src, String dest, int numPassengers) {
        ...
        if (availableSeatCount < numPassengers) {
            bookedSeatsCount = availableSeatCount;
        } else {
            bookedSeatsCount = numPassengers;
        }
        int count = 0;
        for (Coach coach : coaches) {
            if (coach.coachName.startsWith(seatClassPrefix)) {
        ...
        int remainingPassengers = numPassengers - bookedSeatsCount;
        if (remainingPassengers > 0) {
            RouteWaitingList routeWaitingList = getRouteWaitingList(src, dest);
            for (int i = 0; i < remainingPassengers; i++) {
                waitingList.add(routeWaitingList.addWaitingListNumber());
            }
        }
    
        reservedSeats.addAll(waitingList);

        return reservedSeats;
    }

    RouteWaitingList getRouteWaitingList(String src, String dest) {
        String routeKey = src + "-" + dest;
        for (RouteWaitingList routeWaitingList : routeWaitingLists) {
            if (routeWaitingList.getRouteKey().equals(routeKey)) {
                return routeWaitingList;
            }
        }

        RouteWaitingList newRouteWaitingList = new RouteWaitingList(routeKey);
        routeWaitingLists.add(newRouteWaitingList);
        return newRouteWaitingList;
    }
...
class RouteWaitingList {
    private String routeKey;
    private int counter;
    private List<String> waitingList;

    public RouteWaitingList(String routeKey) {
    ...
    public String addWaitingListNumber() {
        String waitingListNumber = "WL-" + counter++;
        waitingList.add(waitingListNumber);
